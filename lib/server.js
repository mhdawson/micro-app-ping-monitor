...
Server.startServer = function(server) {
  ...
  setInterval(function() {
    eventLog.logMessage(config, 'Connectivity check failed', eventLog.LOG_WARN);
    if (connected === false) {
      if (((new Date()) - lastGood) > config.alertTime) {
        if (alarmed === false) {
          eventLog.logMessage(config, 'No connectivity', eventLog.LOG_ERROR);
        }
        alarmed = true;
      }
    } else {
      if (alarmed === true) {
        eventLog.logMessage(config, 'Connectivity restored', eventLog.LOG_INFO);
      }
      alarmed = false;
      lastGood = new Date();
    }
